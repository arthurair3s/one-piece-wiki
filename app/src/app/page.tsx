  ...
  const [islands, setIslands] = useState<Island[]>([
    {
      id: 1,
      name: 'Dawn Island',
      description: 'Ilha natal de Luffy.',
      coordinate_x: 15,
      coordinate_y: 45,
      coordinate_z: 0,
      model_url: '',
    ...
    },
    {
      id: 2,
      name: 'Alvida Ship',
      description: 'Navio onde Coby era prisioneiro.',
      coordinate_x: 35,
      coordinate_y: 25,
      coordinate_z: 0,
      model_url: '',
    ...
    },
    {
      id: 3,
      name: 'Shells Town',
      description: 'Base da Marinha comandada por Morgan.',
      coordinate_x: 55,
      coordinate_y: 40,
      coordinate_z: 0,
      model_url: '',
    ...
    },
    {
      id: 4,
      name: 'Orange Town',
      description: 'Cidade dominada por Buggy.',
      coordinate_x: 75,
      coordinate_y: 20,
      coordinate_z: 0,
      model_url: '',
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (apiSagas) {
          const sagasArray = Array.isArray(apiSagas)
            ? apiSagas
            : (apiSagas as any)?.data
          if (sagasArray && sagasArray.length > 0) setSagas(sagasArray)
        }
        if (apiArcs) {
          const arcsArray = Array.isArray(apiArcs)
            ? apiArcs
            : (apiArcs as any)?.data
          if (arcsArray && arcsArray.length > 0) setArcs(arcsArray)
        }
        if (apiIslands) {
          const islandsArray = Array.isArray(apiIslands)
            ? apiIslands
            : (apiIslands as any)?.data
          if (islandsArray && islandsArray.length > 0) setIslands(islandsArray)
        }
      } catch (err) {
        console.warn(
          'Utilizando dados do banco via fallback estático realista:',
          err
        )
      }
    }
        ...
        <div className="relative w-full h-full max-w-4xl flex items-center justify-center pointer-events-auto">
          <div className="relative w-[800px] h-[500px]">
            {islands.map((island, index) => {
              ...
              const xPos =
                island.coordinates?.x ?? island.coordinate_x ?? 15 + index * 20
              const yPos =
