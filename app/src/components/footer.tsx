...
export function Footer({
  ...
  const progressPercent = currentArcs.length > 1 ? (sliderVal / (currentArcs.length - 1)) * 100 : 0;

  return (
    <footer className={`absolute bottom-4 left-4 right-4 md:bottom-6 md:left-6 md:right-6 max-w-5xl mx-auto w-[calc(100%-2rem)] md:w-[calc(100%-3rem)] rounded-xl border border-border/40 shadow-lg bg-background/80 backdrop-blur-md z-30 transition-all duration-300 ${className}`}>
            ...
            <div className="relative w-full flex items-center h-8">
              ...
              <div className="absolute left-4 right-4 top-0 bottom-0 pointer-events-none z-10">
                {currentArcs.map((arc, index) => {
                  const pct = currentArcs.length > 1 ? (index / (currentArcs.length - 1)) * 100 : 0;
